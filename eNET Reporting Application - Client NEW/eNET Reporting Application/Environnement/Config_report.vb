...
#End Region
    ...
    Private Sub Config_report_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        'Me.Height = Reporting_application.Height - 100
        ...
        If GlobalVariables.reloadByForm1 = False And Exists(rootPath & "\sys\StartupConfig_TreeView.xml") And Welcome.CSIF_version <> 3 Then

            loadStartupConfigTreeV()
        ElseIf GlobalVariables.reloadByForm1 = False And Exists(rootPath & "\sys\" & Reporting_application.username_ & "_TreeView.xml") And Welcome.CSIF_version <> 3 Then
                    ...
                    Case Else
                        If (prefixTemp <> "_ST") And (prefixTemp <> "_MT") Then
                            If st_present = False And mt_present = True Then
                                ...
                                TreeView_machine.Nodes(j).Nodes.Add(machine_tmp, machine_tmp)
                                'TreeView_machine.ImageList.Images.Add(machine_tmp.ToString(), My.Resources.thumb_SMART_430A)
                                cpt += 1

                            ElseIf mt_present = False And st_present = True Then
                                machine_tmp = machine(i)
                                TreeView_machine.Nodes(k).Nodes.Add(machine_tmp, machine_tmp)
                                'TreeView_machine.ImageList.Images.Add(machine_tmp.ToString(), My.Resources.thumb_SMART_430A)
                                cpt2 += 1
                            ...
                            ElseIf mt_present = False And st_present = False Then
                                ...
                                Else
                                    machine_tmp = machine(i)
                                    TreeView_machine.Nodes.Add(machine_tmp, machine_tmp)
                                    'TreeView_machine.ImageList.Images.Add(machine_tmp.ToString(), My.Resources.thumb_SMART_430A)
                                End If

                            Else
                                machine_tmp = machine(i)
                                TreeView_machine.Nodes(j).Nodes(cpt).Nodes.Add(machine_tmp, machine_tmp)
                                'TreeView_machine.ImageList.Images.Add(machine_tmp.ToString(), My.Resources.thumb_SMART_430A)
...
Fin:
        ...
        If Exists(rootPath & "\sys\SetupColor_.csys") Then
            Using reader As StreamReader = New StreamReader(rootPath & "\sys\SetupColor_.csys")
                Reporting_application.chemin_Color = reader.ReadLine()
            End Using
        End If

        If (Reporting_application.chemin_Color = Nothing) Then
            Reporting_application.chemin_Color = Reporting_application.chemin_eNET + "\_SETUP\GraphColor.sys"
        End If
        ...
        Try
            ...
            If Not System.IO.Directory.Exists(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) & "\Resources\") Then
                My.Computer.FileSystem.CreateDirectory(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) & "\Resources\")
            End If
            ...
            For Each status As String In colors.Keys
                ...
                If Not status.IndexOf("\") = 0 Then
                    status = status.Replace("\", " ")
                End If
                b.Save(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) & "\Resources\" & status & ".bmp")
                MyImageList.Images.Add(Image.FromFile(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) & "\Resources\" & status & ".bmp"))
            ...
            Else
                ...
                Try
                    ...
                    If (GlobalVariables.ListOfMachine.Count() > 0) Then
                        ...
                        state = GlobalVariables.ListOfMachine.Item(n.Text.ToString()).Statut.Replace("/", " ")
...
#Region "TREEVIEW"
    Private Sub TreeView1_AfterCheck(ByVal sender As Object, ByVal e As TreeViewEventArgs) Handles TreeView_machine.AfterCheck
        ...
        Else
            'no need to check nodes since its is the dashboard from enet that is diplayed
...
#End Region
    ...
    Private Sub BTN_Create_Click(sender As Object, e As EventArgs) Handles BTN_Create.Click
        ...
        Try
            ...
            Report_BarChart.CB_Report.Items.Clear()
            Report_BarChart.Label5.Text = ""

            If Welcome.CSIF_version = 1 Then

                'Chart1.Palette 
                Report_BarChart.Chart2.Series("Series3").Color = Color.Transparent
                Report_BarChart.Chart2.Series("Series4").Color = Color.Transparent
            ...
            If Report_BarChart.BTN_Unit.Text = "%" Then Report_BarChart.BTN_Unit.Text = "h"
            If Report_BarChart.CB_Consolidate.Checked = True Then Report_BarChart.CB_Consolidate.Checked = False
            ...
            'If DateDiff(DateInterval.Day, DTP_StartDate.Value.Date, DTP_EndDate.Value.Date) > 28 Then
            If DateDiff(DateInterval.Day, dateStart, dateEnd) > 28 Then
                days = 6

                'periode_returned = CSI_LIB.Detailled(DTP_StartDate.Value, DTP_EndDate.Value, read_tree(), True)
                'big_periode_returned = CSI_LIB.Evolution(DTP_StartDate.Value, DTP_EndDate.Value, read_tree(), days, True)
                ...
                'Report_BarChart.Chart2.ChartAreas(0).AxisX.ScaleView.Zoom((89 - Math.Round(DateDiff(DateInterval.Day, DTP_StartDate.Value.Date, DTP_EndDate.Value.Date) / 7) - 1) + 0.5, 88)
                Report_BarChart.Chart2.ChartAreas(0).AxisX.ScaleView.Zoom((89 - Math.Round(DateDiff(DateInterval.Day, dateStart, dateEnd) / 7) - 1) + 0.5, 88)
            End If

            'If (DateDiff(DateInterval.Day, DTP_StartDate.Value.Date, DTP_EndDate.Value.Date) < 30) Then
            If (DateDiff(DateInterval.Day, dateStart, dateEnd) < 30) Then
                ...
                periode_returned = CSI_LIB.Detailled(dateStart, dateEnd, read_tree(), True)
                big_periode_returned = CSI_LIB.Evolution(dateStart, dateEnd, read_tree(), days, True)
                ' big_periode_returned(0, 1).date_ = "2 2 8 8 2016 2016"
                Call Fill_Combo_Report_BarChart(periode_returned)
            ...
            If DateDiff(DateInterval.Day, dateStart, dateEnd) = 0 Then
                If dateStart.DayOfWeek > 1 Then
                    Report_BarChart.Chart2.ChartAreas(0).AxisX.ScaleView.Zoom((89 + Val(Reporting_application.week_(0)) - dateStart.DayOfWeek) + 0.5, 89.5)
                    'Form9.Chart2.ChartAreas(0).AxisX.ScaleView.Zoom(0, 89.5)
                Else
                    Report_BarChart.Chart2.ChartAreas(0).AxisX.ScaleView.Zoom((89 + Val(Reporting_application.week_(0)) - dateStart.DayOfWeek) - 0.5 - 7 + Val(Reporting_application.week_(0)), 89.5)
                End If
            End If

            ' +1Day AND < 28 DAYS
            If DateDiff(DateInterval.Day, dateStart, dateEnd) < 30 And DateDiff(DateInterval.Day, dateStart, dateEnd) <> 0 Then
                Report_BarChart.Chart2.ChartAreas(0).AxisX.ScaleView.Zoom((89 - DateDiff(DateInterval.Day, dateStart, dateEnd) - 0.5), 89.5)
            End If

            ' +28 DAYS
            If DateDiff(DateInterval.Day, dateStart, dateEnd) > 29 Then
                Report_BarChart.Chart2.ChartAreas(0).AxisX.ScaleView.Zoom((89 - Math.Round(DateDiff(DateInterval.Day, dateStart, dateEnd) / 7) - 1) + 0.5, 89.5)
            End If

            '   End If
            Report_BarChart.CB_Report.SelectedIndex = Report_BarChart.CB_Report.Items.Count - 1
            ...
            ''''''''''''''''''''NEW REPORT USING SQLITE
            'Dim reportdata As New DataTable()
            'reportdata = CSI_LIB.GetSQLiteData("HARDINGE", DTP_StartDate.Value.Date, DTP_EndDate.Value.Date)

            ''Dim charttest As New System.Windows.Forms.DataVisualization.Charting.Chart()
            'Dim machines As String()
            'machines(0) = "MC1"
            'machines(1) = "MC2"
            'Dim testchart As New Report_BarChart_SQLITE_UC(machines)
...
#Region "FORM 7 & AND 9"
    ...
    Public Sub fill_Report_BarChart_shift(ByRef periode_returned As periode(), i As Integer)
        ...
        If (total <> 0) Then
            ...
            If Report_BarChart.BTN_Unit.Text = "h" Then
    ...
    Public Sub Fill_Combobox_detailled(ByRef periode_ As periode())
        ...
        If ((UBound(periode_) - 1)) = 0 Then
            index = 0
        Else
            index = Report_BarChart.CB_Report.SelectedIndex
        End If
    ...
    Public Sub Fill_Combobox_partnumber(ByRef periode_ As periode())
        ...
        For i = 0 To (UBound(periode_) - 1)
            ...
            End If
        Next
        Dim index As Integer = Report_BarChart.CB_Report.SelectedIndex
        ...
        If Not IsNothing(period_return) Then
            ...
            For i = 0 To (UBound(period_return) - 1)
                If period_return.Length <= machineList.Length Then
                    Report_BarChart.CB_Report.Items.Add(period_return(i).machine_name & " : " & period_return(i).date_)
                Else
                    Report_BarChart.CB_Report.Items.Add(machineList(i) & " : " & period_return(i).date_)
                End If
            Next
        ...
        Try
            ...
            If (machine.Count) > 0 Then
                ...
                For i = 0 To UBound(machine)
                    ...
                    If Welcome.CSIF_version = 3 Then
                        ' command = "SELECT  DISTINCT  status, year_, month_, day_ ,'" + CSILIB.RealNameMachine(machine(i)) + "' as machineName FROM  tbl_" & machine(i) & " WHERE status LIKE '_PARTNO:%'"
                        command = "SELECT  DISTINCT  substring(status,9), date(Date_),'" + CSILIB.RealNameMachine(machine(i)) + "' as machineName FROM  tbl_" & machine(i) & " WHERE status LIKE '_PARTNO:%'"

                        Dim mySqlComm As New MySqlDataAdapter(command, cnt_SQL)
                        mySqlComm.Fill(tbl_temp2)
