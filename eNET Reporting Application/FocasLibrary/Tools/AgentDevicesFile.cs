...
namespace FocasLibrary.Tools
{
    public static class AgentDevicesFile
    {
        ...
        public static void WriteDeviceNodeForAgent(string agentDevicesFile,XmlNode node)
        {

            try
            {
                if (File.Exists(agentDevicesFile))
                {
                    var doc = XML.Files.ReadDocument(agentDevicesFile);
                    if (doc != null)
                    {
                        var root = doc.DocumentElement;
                        ...
                        XML.Files.WriteDocument(doc, agentDevicesFile);
        ...
        public static void DeleteDeviceNodeForAgent(string agentDevicesFile,string xPath)
        {
            if (File.Exists(agentDevicesFile))
            {
                var doc = XML.Files.ReadDocument(agentDevicesFile);
                if (doc != null)
                {
                    ...
                    if (node != null && devicesNode != null)
                    {
                        ...
                        XML.Files.WriteDocument(doc, agentDevicesFile, true);
                    }
                }
            }
        }
        ...
        public static XmlNode CreateDeviceNode(DeviceInfo info)
        {
            ...
            if (File.Exists(Paths.DEVICE_TEMPLATE))
            {
                var doc = XML.Files.ReadDocument(Paths.DEVICE_TEMPLATE);
                if (doc != null)
                {
                    XML.Attributes.Set(doc, "/Device", "name", info.DeviceName);
                    XML.Attributes.Set(doc, "/Device", "id", info.DeviceId);
                    //XML.Attributes.Set(doc, "/Device", "uuid", info.DeviceName);
                    ...
                    //{
                    //    var id = XML.Attributes.Get(node, "id");
                    //    if (id != null && id.Contains("[dev_id]"))
                    //    {
                    //        id = id.Replace("[dev_id]", info.DeviceId);
        ...
        public static List<DeviceInfo> ReadDeviceInfos(string devicesFile)
        {
            var result = new List<DeviceInfo>();

            if (File.Exists(devicesFile))
            {
                var doc = XML.Files.ReadDocument(devicesFile);
                if (doc != null)
                {
                    var nodes = doc.SelectNodes("//Device");
                    foreach (XmlNode node in nodes)
