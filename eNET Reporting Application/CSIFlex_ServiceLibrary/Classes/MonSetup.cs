        ...
        public void updateMonsetupFiles(string[] filenames, Stopwatch watch)
        {
            ...
            for (int filein = 0; filein < len; filein++)
            {
                ...
                if (dtreader12.Rows.Count > 0)
                {
                    ...
                    //DateTime datetimeofprev = Convert.ToDateTime(dtreader12.Rows[1]["correction_date_"]); // If we have two rows then we take this in count 
                        ...
                        if (datetimemon != datetime)
                        {
                            if (status == "_COFF")
                            {
                                ...
                                if (dtq33.Rows.Count > 1) // Means we have two rows
                                {
                                    ...
                                    if (oldstatus == "_CON")
                                    {
                                        Utility.WriteToFile("First Entry :: Here Part Count Added Successfully");
                                        ...
                                        DateTime datetimeofprev = Convert.ToDateTime(dtq33.Rows[1]["correction_date_"]);
