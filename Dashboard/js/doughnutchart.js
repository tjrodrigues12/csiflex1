...
function initializeDoughnutChart() {
    ...
    Chart.pluginService.register({
        beforeDraw: function (chart) {
            ...
            if(data.datasets[0].data[0] == 0 || totalOfValues == 0)
                {
                    ctx.fillText("No data", cycleOntextX, cycleOntextY - 10);
                }
            else
                {
                    ctx.fillText(cycleOnString, cycleOntextX, cycleOntextY - 10);
                    var datapercent = (data.datasets[0].data[0] / totalOfValues);
                    ctx.fillText((datapercent * 100).toFixed(2) + "%", cycleOntextX + 5, cycleOntextY + 10);
                }
            ctx.save();
        }
    });
