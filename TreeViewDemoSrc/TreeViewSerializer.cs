	...
	public class TreeViewSerializer
	{
		...
		public void DeserializeTreeView(TreeView treeView, string fileName)
		{
            ...
            try
			{
				...
				while (reader.Read())
				{
					...
					{						
						if (reader.Name == XmlNodeTag)
						{
                            ...
                            if (!isFirst && newNode.Checked == false)
                            {
                                allChecked = false;
                            }

                            if (isFirst && isEmptyElement)
                            {
                                TreeNode tempNode = new TreeNode();
                                SetAttributeValue(tempNode, "text", "MACHINES");
                                SetAttributeValue(tempNode, "name", "");
                                SetAttributeValue(tempNode, "imageindex", "-1");
                                SetAttributeValue(tempNode, "checked", "FALSE");
                ...
                if (allChecked && treeView.Nodes.Count > 0)
                {                    
                    SetAttributeValue(treeView.Nodes[0], "checked", "TRUE"); 
                }
			}
			finally
			{
		...
		private void SetAttributeValue(TreeNode node, string propertyName, string value)
		{
            ...
            else if (propertyName == XmlNodeChecked)
            {
                node.Checked = bool.Parse(value);
            }
            else if (propertyName == XmlNodename)
            {
                node.Name  = value;
            }
