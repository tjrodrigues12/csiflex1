...
FanucAdapter::FanucAdapter(int aPort)
  : Adapter(aPort), mAvail("avail"), mMessage("message"), mPartCount("part_count"), mPartRequired("Required_Part"), mTotalCount("Part_Count_Total"),
...
void FanucAdapter::getCounts()
{
  ...
  short ret2 = cnc_rdparam(mFlibhndl, 6712, 0, 8, &buf);
  if (ret == EW_OK)
  {
	  mTotalCount.setValue(buf.u.ldata);
  }

  short ret1= cnc_rdparam(mFlibhndl, 6713, 0, 8, &buf);
  if (ret == EW_OK)
  {
	  mPartRequired.setValue(buf.u.ldata);
	  //printf("new parameter is working");
  }
}
