...
Public Class form_raw_values
    ...
    Dim overrideSource As BindingSource
    ...
    Private machineName_ As String
    ...
    Private _shiftStart As DateTime
    ...
    Private Sub form_raw_values_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        overrideSource = New BindingSource()
        overrideSource.DataSource = overrideItens

        dgvOverride.AutoGenerateColumns = False
        dgvOverride.DataSource = overrideSource
...
#Region "Region - Raw Data Tab"
    ...
    Private Sub LoadDgvTimeline()
        ...
        sqlCmd.Append($" WHERE                                    ")
        sqlCmd.Append($"      ShiftDate >= '{ dateTimeline_.ToString("yyyy-MM-dd") }' ")
        sqlCmd.Append($"  AND ShiftDate <  '{ dateNextDay.ToString("yyyy-MM-dd") }'   ")
        sqlCmd.Append($"  AND shift =  { shift_ }                 ")
        sqlCmd.Append($"  AND status NOT LIKE '\_%'               ")
        sqlCmd.Append($" ORDER BY ShiftDate                       ")

        Dim dtTimeline = MySqlAccess.GetDataTable(sqlCmd.ToString(), connString)

        If dtTimeline.Rows.Count = 0 Then

            MessageBox.Show("This shift doesn't have raw data to be displayed.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Me.Close()
            Return
        End If
    ...
    Private Sub btnExportRawData_Click(sender As Object, e As EventArgs) Handles btnExportRawData.Click
        DGVExportCsv.DataGridViewExportCsv(dgvTimeline, $"{machineName_}_RawData_{ShiftStart.ToString("yyyyMMdd_HHmm")}")
    End Sub
    ...
    Private Sub btnApply_Click(sender As Object, e As EventArgs) Handles btnApply.Click
        ...
        Else
            ...
            If Not hasSelection Then
                ...
                If dgvTimeline.SelectedRows.Count > 0 Then

                    Dim timelineItem As TimelineItem = timelineItems.FirstOrDefault(Function(m) m.TimeId = dgvTimeline.SelectedRows(0).Cells("TimeId").Value)
                    Dim change = New TimelineItem()
                    timelineItem.CopyPropertiesTo(change)
                    change.Action = "E"
                    change.OriginalComments = timelineItem.Comments
...
#Region "Region - Override Tab"

    Private Sub LoadOverride()
        ...
        sqlCmd.Append($" WHERE                                    ")
        sqlCmd.Append($"      MachineId     = { machineId }       ")
        sqlCmd.Append($"  AND EventDateTime >= '{ _shiftStart.ToString("yyyy-MM-dd HH:mm:ss") }' ")
        sqlCmd.Append($"  AND EventDateTime <  '{ _shiftEnd.ToString("yyyy-MM-dd HH:mm:ss") }'   ")
        sqlCmd.Append($" ORDER BY EventDateTime                   ")

        Dim dtOverride = MySqlAccess.GetDataTable(sqlCmd.ToString(), connString)
        ...
        For Each rowChange As DataRow In dtOverride.Rows

            Dim change As OverrideItem = New OverrideItem()

            change.EventDateTime = DateTime.Parse(rowChange("EventDateTime").ToString())
            change.Status = rowChange("SystemStatus")
            change.PartNumber = IIf(Not IsDBNull(rowChange("PartNumber")), rowChange("PartNumber"), "")
            change.Operation = IIf(Not IsDBNull(rowChange("Operation")), rowChange("Operation"), "")
            change.Feedrate = IIf(Not IsDBNull(rowChange("FeedOverride")), rowChange("FeedOverride"), lastFeedrate)
            change.Rapid = IIf(Not IsDBNull(rowChange("RapidOverride")), rowChange("RapidOverride"), lastRapid)
            change.Spindle = IIf(Not IsDBNull(rowChange("SpindleOverride")), rowChange("SpindleOverride"), lastSpindle)
        ...
        If overrideItens.Count > 0 Then
            cycleTime = _shiftEnd.TimeOfDay.TotalSeconds - lstDatetime.TimeOfDay.TotalSeconds
            If cycleTime < 0 Then cycleTime += 86400
        ...
        overrideSource.ResetBindings(False)
    ...
    Private Sub LoadOverrideChart()
        ...
        chartOverride.ChartAreas(0).AxisY.MajorGrid.Enabled = True
        chartOverride.ChartAreas(0).AxisY.MajorGrid.LineColor = Color.SkyBlue

        chartOverride.ChartAreas(0).AxisY.Title = ""
        chartOverride.ChartAreas(0).AxisY.IntervalType = DateTimeIntervalType.Number
        chartOverride.ChartAreas(0).AxisY.LabelStyle.Format = "{0}%"
        chartOverride.ChartAreas(0).AxisY.Enabled = AxisEnabled.True
        chartOverride.ChartAreas(0).AxisY.Maximum = 150
        chartOverride.ChartAreas(0).AxisY.Interval = 20
        ...
        'Dim ConColor = Color.FromArgb(&HE8F5E9)
        'Dim OthColor = Color.FromArgb(&HFFFDE7)
        'Dim tmpColor As Color
        'For Each item As OverrideItem In overrideItens

        '    tmpColor = IIf(item.Status = "CYCLE ON", ConColor, OthColor)

        '    chartOverride.Series("Status").Points.AddXY(item.EventDateTime.ToOADate, 100)
        '    chartOverride.Series("Status").Points(chartOverride.Series("Status").Points.Count - 1).Color = tmpColor

        '    chartOverride.Series("Status").Points.AddXY(item.EventDateTime.AddSeconds(item.CycleTime).ToOADate, 100)
        '    chartOverride.Series("Status").Points(chartOverride.Series("Status").Points.Count - 1).Color = tmpColor
        'Next
    ...
    Private Sub chkFeedrate_CheckedChanged(sender As Object, e As EventArgs) Handles chkFeedrate.CheckedChanged
        ...
        If chkFeedrate.Checked And chartOverride.Series.IndexOf("Feedrate") = -1 Then
            ...
            serieFeedrate.YAxisType = AxisType.Primary
    ...
    Private Sub chkRapid_CheckedChanged(sender As Object, e As EventArgs) Handles chkRapid.CheckedChanged
        ...
        If chkRapid.Checked And chartOverride.Series.IndexOf("Rapid") = -1 Then
            ...
            serieRapid.YAxisType = AxisType.Primary
    ...
    Private Sub chkSpindle_CheckedChanged(sender As Object, e As EventArgs) Handles chkSpindle.CheckedChanged
        ...
        If chkSpindle.Checked And chartOverride.Series.IndexOf("Spindle") = -1 Then
            ...
            serieSpindle.YAxisType = AxisType.Primary
        ...
        For Each result In results
            ...
            If result.ChartElementType = ChartElementType.DataPoint Then
                ...
                If prop IsNot Nothing Then
                    ...
                    If (Math.Abs(pos.X - pointXPixel) < 2 And Math.Abs(pos.Y - pointYPixel) < 2) Then
                        ...
                        Try
                            ...
                            ToolTip1.Show(legend.ToString(), chartOverride, pos.X + 10, pos.Y - 15)
    ...
    Private Sub btnOVRExportCsv_Click(sender As Object, e As EventArgs) Handles btnOVRExportCsv.Click
        DGVExportCsv.DataGridViewExportCsv(dgvOverride, $"{machineName_}_Override_{ShiftStart.ToString("yyyyMMdd_HHmm")}")
    End Sub

    Private Sub chartOverride_Click(sender As Object, e As MouseEventArgs) Handles chartOverride.Click
...
'ChartArea3.InnerPlotPosition.Height = 58.01672!
'ChartArea3.InnerPlotPosition.Width = 99.92!
