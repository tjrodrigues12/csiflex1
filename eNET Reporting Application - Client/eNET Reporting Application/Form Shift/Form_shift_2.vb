...
Public Class Form_shift_2
    ...
    Private Sub Form_shift_2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim time_ As DateTime, shift As New DataView(Report_PieChart.Timeline2)
        ...
        Try
            ...
            If start_shift.ToOADate > end_shift.ToOADate Then end_shift = DateAdd(DateInterval.Hour, 24, end_shift)
            '
            'If start_shift.ToOADate > end_shift.ToOADate Then
            '    Dim d As Date = Date.FromOADate(end_shift.ToOADate)
            '    d = d.AddDays(1)
            '    end_shift = d
            'End If
            ...
            Chart1.ChartAreas(0).AxisX.Minimum = start_shift.ToOADate
            '  Chart2.ChartAreas(0).AxisX.Minimum = start_shift.ToOADate
            ...
            While (starthour < endhour) And (last_loop = False)
                ...
                shift.RowFilter = String.Format(CultureInfo.InvariantCulture.DateTimeFormat,
                     ...
                     "Time_ <= #{0}#", hour.AddHours(1)) & " and Shift=" & 2
                ...
                If PreviousCycleName <> "" And Not (status.Contains("_PARTNO") Or status.Contains("_SH_START") Or status.Contains("_SH_END")) Then
                    If PreviousCycle > DateDiff(DateInterval.Second, PreviousCycleHour, hour.AddHours(1)) And overshoot = 0 Then
                        ...
                        '  Hourly_stat(item_ - 1).shift1.Item(PreviousCycleName) = Hourly_stat(item_ - 1).shift1.Item(PreviousCycleName) - PreviousCycle + DateDiff(DateInterval.Second, PreviousCycleHour, hour)
            ...
            Dim color_list As Dictionary(Of String, String) = Report_PieChart.colors
            ...
            Me.Text = "Machine : " & combovalue(0) & "   -   Shift 2, Date : " & combovalue(1)
...
#Region "Click"
    ...
    Private Sub clicked(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        'Dim shift As New DataView(Form7.Timeline2)
