            ...
            Using sqlConn As MySqlConnection = New MySqlConnection(connectionString)
                ...
                If machine_.Length > 0 Then
                    For i = 0 To UBound(machine_)
                        ...
                        command = "SELECT '" + realNameMachine_(i) + "' as MchName, date(Date_) as date, Mid(Status, 9)  as partName, count(Status) as counting " +
                            " FROM  tbl_" & machine_(i) & "  " +
                            " WHERE Status LIKE '_PARTNO:%' and ShiftDate between '" + dateStart + "' and '" + dateEnd + "'  " +
                            "group by Status, date(ShiftDate) "
            ...
            Using sqlConn As MySqlConnection = New MySqlConnection(connectionString)
                ...
                If machine_.Length > 0 Then
                    For i = 0 To UBound(machine_)
                        ...
                        command = "SELECT '" + realNameMachine_(i) + "' as MchName, Mid(Status, 9)  as partName, count(Status)*1 as occurrence, round((count(Status)*1.0 / (SELECT (count(Status)*1.0) as percentage  FROM  tbl_" & machine_(i) & "   WHERE Status LIKE '_PARTNO:%' and ShiftDate between '" + dateStart + "' and '" + dateEnd + "') * 100.0), 2) as percentage " +
                            " FROM  tbl_" & machine_(i) & "  " +
                            " WHERE Status LIKE '_PARTNO:%' and ShiftDate between '" + dateStart + "' and '" + dateEnd + "'  " +
                            "group by Status order by percentage desc"
