...
Public Class Reporting_partsNumber
    ...
    Private Function setPartNo() As DataTable
        ...
        Try
            ...
            Else
                ...
                connectionString = CSIFLEXSettings.Instance.ConnectionString
                ...
                Using sqlConn As MySqlConnection = New MySqlConnection(connectionString)
                    ...
                    If machine.Length > 0 Then
                        For i = 0 To UBound(machine)
                            ...
                            command = $"SELECT '{ realMachine(i) }' as MchName, DATE_FORMAT(shiftdate, '%m-%d-%Y') as 'date', Mid(Status, 9)  as partName, count(Status) as 'count' "
                            command += $" FROM  tbl_{ machine(i) } "
                            command += $" WHERE Status LIKE '_PARTNO:%' and shiftdate between '{ startdate.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-US")) }' AND '{ enddate.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-US")) }' "
                            command += $"    GROUP BY Status, DATE_FORMAT(shiftdate, '%m-%d-%Y') "

                            temp_table = MySqlAccess.GetDataTable(command, connectionString)
                            ...
                            Try
                                ...
                                Next
                            Catch ex As Exception
                                Log.Error("Unable to merge parts table", ex)
                            End Try
        ...
        Catch ex As Exception
            ...
            Log.Error("Unable to get part informations", ex)
        End Try
    ...
    Private Sub generateCSV_file()
        ...
        Catch ex As Exception
            ...
            Log.Error("Unable to generate part report csv", ex)
        End Try
