...
function initTimelines() {
    ...
    // console.log('userConfig.TimeLineBarHeight',userConfig[0].TimeLineBarHeight)

    timelineHeightAjust = (userConfig[0].TimeLineBarHeight / 100)

    // console.log('timelineHeightAjust',timelineHeightAjust)

    timelineHeight = $("#hiddendiv").html("test").outerHeight() * timelineHeightAjust; //60% of cell height
    $("#hiddendiv").html("");
        ...
        try {
            ...
            if (timelinesHashmap[mach_values[rec].Machine].shift == mach_values[rec].Shift) {
                ...
                if (timelinesHashmap[mach_values[rec].Machine].currentstatus == mach_values[rec].Status) {
                    ...
                    if((lastwidth + elapsedtime) >= 0)
                        lastrect.setAttributeNS(null, 'width', (lastwidth + elapsedtime)); //cycletime in seconds
                    else
                        lastrect.setAttributeNS(null, 'width', 0); //cycletime in seconds
