...
/*global $, userConfig, iframedelay, livestatusdelay:true,  timelineEnabled:true, trendEnabled:true, legendminheight:true, legendmodulo:true, rowqty, currentPerfGroups, DisplayableGroupsPerPage, livestatusHashmap, groups, lstimer:true, paused, lowanimation:true, dateformat:true, lsByGroup:true*/
...
function updateUIOptions() {
    ...
    if (userConfig[0].Browserzoom != 100) {
        var emscale = parseInt(userConfig[0].Browserzoom);
        $('body').css('zoom', emscale + '%');
    }
    ...
    if (!groups) {
        ...
        if (userConfig[0].DisplayWhat === "DisplayPerf") {
            ...
            $('#targetid').show();
        } else if (userConfig[0].DisplayWhat === "Both") {
            $('#currentperfid').show();
            $('#targetid').show();
    ...
    if (userConfig[0].FeedrateOver === "off") {
        $('.colfovr').hide();
    }
    if (userConfig[0].SpindleOver === "off") {
        $('.colsovr').hide();
    }
    if (userConfig[0].RapidOver === "off") {
        $('.colrovr').hide();
    }
    ...
    // lsByGroup = (userConfig[0].lsByGroup === 'True' ? true : false);

    timelineEnabled = (userConfig[0].timeline === '0' ? false : true);
    trendEnabled = (userConfig[0].trends === '0' ? false : true);
    lsByGroup = (userConfig[0].lsByGroup === 'True' ? true : false);
...
function panelRotation() {
    if (livestatusdisplayed) {
        ...
        setTimeout(panelRotation, livestatusdelay);
    } else {
        $("#iframeid").hide();
